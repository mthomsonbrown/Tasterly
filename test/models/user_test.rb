...
class UserTest < ActiveSupport::TestCase
  ...
  ### 
  ### First legitimate red test!
  ###
  test "can't add friendship with friend id that is not a user id" do
    bogus = 42
    user = User.find(1)
    assert user.friendships.size == 0
    user.friendships.create! friend_id: bogus
    assert user.friendships.size == 0, "frienship size wasn't 0, was " +
    "#{user.friendships.size}, and friend_id was bogus test id is "+
    "#{user.friendships.first.friend_id == bogus}"
  end
  ###
  ###
  ###
